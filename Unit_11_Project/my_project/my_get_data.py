...
def prepare_image(image: np.ndarray, x: int, y: int, width: int, height: int, size: int) -> \
    ...
    target_array = image.copy()  # image[area].copy()
...
class RandomImagePixelationDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with Image.open(self.image_files[index]) as im:
            image = np.array(resize_transforms(im), dtype=self.dtype)
            #print(image.size)
        #with Image.open(self.image_files[index]) as im:
         #   image = np.array(im, dtype=self.dtype)  # self.dtype)
            # print(image.shape())
        image = to_grayscale(image)  # Image shape is now (1, H, W)
        image_width = image.shape[-1]  # always 64 in our trainingsdata
        image_height = image.shape[-2]  # always 64 in our trainingsdata
